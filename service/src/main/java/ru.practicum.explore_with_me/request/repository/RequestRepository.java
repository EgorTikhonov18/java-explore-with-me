...
public interface RequestRepository extends JpaRepository<Request, Long> {
   @Query(value = "select r from Request r where r.requester = :requester")
    List<Request> findRequestByRequester( @Param("requester") User requester); //@Param("requester")

    @Query(value = "select r from Request r where r.event = :event")
    List<Request> findRequestsByEventAndRequester( @Param("event") Event event ); //@Param("ru.practicum.explore_with_me/event")

     @Query(value = "select r from Request r where r.id in :requestIds")
   List<Request> findRequestsByRequestIds( @Param("requestIds") List<Long> requestIds); //@Param("requestIds")
