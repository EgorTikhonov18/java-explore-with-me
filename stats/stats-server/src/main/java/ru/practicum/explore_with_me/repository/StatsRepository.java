...
public interface StatsRepository extends JpaRepository<EndpointHit, Long> {
    ...
    @Query("select new ru.practicum.explore_with_me.model.ViewStats(hit.app, hit.uri, count(distinct hit.ip)) " +
    ...
    @Query("select new ru.practicum.explore_with_me.model.ViewStats(hit.app, hit.uri, count(hit.ip)) " +
            "from EndpointHit hit " +
            "where (hit.timestamp between :start and :end) " +
            "and hit.uri IN (:uris) " +
            "group by hit.app, hit.uri " +
    ...
    @Query(value = "select new ru.practicum.explore_with_me.model.ViewStats(hit.app, hit.uri, count(hit.ip)) " +
            "from EndpointHit hit " +
            "where (hit.timestamp between :start and :end) " +
            "group by hit.app, hit.uri " +
            "order by count(hit.ip) desc")
